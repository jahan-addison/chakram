...
Node_Root = Union[Operator_Type, str, float, int]
...
class AST_Transformer(Transformer):
    ...
    def vector_definition(self, args) -> AST_Node:
        ...
        return self.__construct_node(
            args, "vector_definition", args[0].value, left=args[1], right=args[2:]
        )
    ...
    def __construct_statement_node(
        self,
        token: Union[List[Tree], List[Token], AST_Node],
        root: Node_Root,
        left: Optional[Node] = None,
    ...
    def switch_statement(self, args) -> AST_Node:
        return self.__construct_statement_node(
            args, "switch", left=args[0], right=args[1:]
        )

    def case_statement(self, args) -> AST_Node:
        return self.__construct_statement_node(
            args, "case", left=args[0], right=args[1:]
        )
    ...
    def while_statement(self, args) -> AST_Node:
        return self.__construct_statement_node(
            args, "while", left=args[0], right=args[1:]
        )
    ...
    def extrn_statement(self, args) -> AST_Node:
        return self.__construct_statement_node(
            args,
            "extrn",
            left=list(map(lambda lvalue: self.__to_identifier(lvalue), args)),
        )
    ...
    def ternary_expression(self, args) -> AST_Node:
        return self.__construct_node(
            args, "ternary_expression", args[0], left=args[1], right=args[2]
        )
    ...
    def assignment_expression(self, args) -> AST_Node:
        return self.__construct_node(
            args, "assignment_expression", args[1], left=args[0], right=args[2]
        )
    ...
    def vector_identifier(self, args) -> AST_Node:
        node = self.__construct_node(
            args, "vector_lvalue", args[0]["root"], left=args[1]
        )
        name = args[0]["root"]
        if name in self._symbol_table:
            self._symbol_table[name]["type"] = "vector_lvalue"
            if (
                isinstance(args[1]["root"], int)
                and "size" not in self._symbol_table[name]
            ):
    ...
    def double_literal(self, args) -> AST_Node:
        number = f"{args[0]}.{args[1]}"
        return self.__construct_constant_node(args, "double_literal", float(number))
