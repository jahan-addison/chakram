...
from .fixture.program_1_ast import (
    program_example_1_ast,
    program_simple_ast_symbols,
    program_example_1_ast_meta,
)
from .fixture.program_1_ast import (
    program_float_ast_as_json,
    program_simple_ast_symbols_as_json,
    program_example_1_ast_symbols_as_json,
    program_example_1_ast_symbols,
    program_simple_ast_symbols,
)
...
def test_get_source_program_as_ast(
    program_example_1_ast: str, program_example_1_ast_meta: str
) -> None:
    with open(getcwd() + "/examples/1.b") as file:
        contents = file.read()
        assert str(get_source_program_as_ast(contents)) == program_example_1_ast
        assert (
            str(get_source_program_as_ast(contents, meta=True))
            == program_example_1_ast_meta
        )
...
def test_get_source_floats_program_ast_as_json(program_float_ast_as_json: str) -> None:
    import json

    with open(getcwd() + "/test/fixture/floats.b") as file:
        test_contents = file.read()
        test = json.dumps(get_source_program_as_ast(test_contents))
        assert test == program_float_ast_as_json


def test_get_source_program_symbol_table(
    program_simple_ast_symbols_as_json: str,
) -> None:
...
def test_get_source_program_symbol_table_as_json(
    program_example_1_ast_symbols_as_json: str,
) -> None:
    with open(getcwd() + "/test/fixture/bad.b") as file:
        test_contents = file.read()
        with pytest.raises(Syntax_Error):
